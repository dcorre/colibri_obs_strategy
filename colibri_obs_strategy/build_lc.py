...
class obs_strategy:
    ...
    def __init__(
        self, wvl_step=100, wvl_min=2500, wvl_max=25000, plot=True, resdir="Default"
    ):
        """
        Class Constructor.
    ...
    def set_strategy_obs(
        self,
        load_strategy=False,
        filename="Default_strategy.hjson",
        t_since_burst=60,
        t_start=0,
        t_end=300,
        t_dead=5,
        ...
        if load_strategy:
            ...
            # Setting the srategy:
            self.t_since_burst = strategy_file[
                "T_since_trigger"
            ]  # Time since burst in seconds (t0), in seconds
            ...
            if self.nb_channels == 3:
                self.list_channels = ["vis1", "vis2", "nir"]
            elif self.nb_channels == 2:
                self.list_channels = ["vis", "nir"]
            counter_channel = 1
            for i in range(len(strategy_file["strategy"])):
                ...
                for channel in self.list_channels:
                    ...
                    band = list(strategy_file["strategy"]["%d" % i][channel].items())[
                        0
                    ][0]
                    # print (band)
                    for nb_exp in range(
                        strategy_file["strategy"]["%d" % i][channel][band]
                    ):
                        tot_nb_exp = strategy_file["strategy"]["%d" % i][channel][band]
                        nb_exp += 1  # Python indexing starts at 0
                        obs_name_table.append(None)
                        texp_table.append(
                            int((Texp - (tot_nb_exp - 1) * self.t_dead) / tot_nb_exp)
                        )
                        t_since_burst_table.append(
                            current_time
                            + int(
                                (
                                    (Texp - (tot_nb_exp - 1) * self.t_dead) / tot_nb_exp
                                    + self.t_dead
                                )
                                * (nb_exp - 1)
                            )
                ...
                current_time += (
                    strategy_file["strategy"]["%d" % i]["Texp"] + self.t_dead
                )
            ...
            # Setting the srategy:
            self.t_since_burst = (
                t_since_burst  # Time since burst in seconds (t0), in seconds
            )
            ...
            for t_start in starting_time_obs:
                ...
                if self.nb_channels == 3:
                    strategy_dict["Observation_sets"][obs_number] = {
                        "t_start": t_start,
                        "texp": texp[filter_set],
                        "filter": {
                            self.filter_vis1[filter_set][channel1],
                            self.filter_vis2[filter_set][channel2],
                            self.filter_nir[filter_set][channel3],
                        },
                        "obs_mag": {
                            self.filter_vis1[filter_set][channel1]: None,
                            self.filter_vis2[filter_set][channel2]: None,
        ...
        ascii.write(
            self.strategy_obs,
            self.respath + "Observational_strategy.txt",
            overwrite=True,
        )
        ascii.write(
            self.strategy_obs,
            self.respath + "Observational_strategy.tex",
            format="latex",
            overwrite=True,
        )
    ...
    def set_grb_params(
        ...
        random_flag=False,
        load_params=False,
        params_file="grb_params_emp.tex",
        load_min=1,
        load_max=10,
        load_distrib=True,
        distrib_file="Turpin_GS02.txt",
        num_samples=1,
        model="gs02",
        ...
        if load_params == True:
            ...
            if load_distrib == True:
                distrib = ascii.read(
                    self.path + "/distrib/" + distrib_file,
                    delimiter="\t",
                    fill_values=[("-", np.nan)],
                )
            ...
            if model == "kann":
                if random_flag == False:
                    ...
                    if load_distrib and "z" in distrib.colnames:
                        hist = np.histogram(
                            ...
                            bins=20,
                            density=True,
                            range=(np.min(distrib["z"]), np.max(distrib["z"])),
                        )
                    ...
                    if load_distrib and "Av" in distrib.colnames:
                        hist = np.histogram(
                            ...
                            bins=20,
                            density=True,
                            range=(np.min(distrib["Av"]), np.max(distrib["Av"])),
                        )
                        hist_dist = rv_histogram(hist)
                    ...
                    if load_distrib and "ExtLaw" in distrib.colnames:
                        extlaws_samples = random.choices(
                            distrib["ExtLaw"], k=num_samples
                        )
                    else:
                    ...
                    if load_distrib and "Rc" in distrib.colnames:
                        hist = np.histogram(
                            ...
                            bins=13,
                            density=True,
                            range=(np.min(distrib["Rc"]), np.max(distrib["Rc"])),
                        )
                        hist_dist = rv_histogram(hist)
                    ...
                    if load_distrib and "beta" in distrib.colnames:
                        hist = np.histogram(
                            ...
                            bins=13,
                            density=True,
                            range=(np.min(distrib["beta"]), np.max(distrib["beta"])),
                        )
                        hist_dist = rv_histogram(hist)
                    ...
                    if load_distrib and "alpha" in distrib.colnames:
                        hist = np.histogram(
                            ...
                            bins=13,
                            density=True,
                            range=(np.min(distrib["alpha"]), np.max(distrib["alpha"])),
                        )
                        hist_dist = rv_histogram(hist)
                    ...
                    if load_distrib and "wvl0" in distrib.colnames:
                        hist = np.histogram(
                            ...
                            bins=13,
                            density=True,
                            range=(np.min(distrib["wvl0"]), np.max(distrib["wvl0"])),
                        )
                        hist_dist = rv_histogram(hist)
                    ...
                    if load_distrib and "t0" in distrib.colnames:
                        hist = np.histogram(
                            ...
                            bins=13,
                            density=True,
                            range=(np.min(distrib["t0"]), np.max(distrib["t0"])),
                        )
                        hist_dist = rv_histogram(hist)
                ...
                for mag, z, be, al in list(zip(Rc, z_samples, beta, alpha)):
                    ...
                    F_z1_nu_t = (
                        ...
                        * (1 + z)
                        / (1 + z0)
                        * (
                            cosmo.luminosity_distance(z0).value
                ...
                if random_flag == False:
                    ...
                    if load_distrib == True and "n0" in distrib.colnames:
                        hist = np.histogram(
                            ...
                            density=True,
                            range=(
                                np.log10(np.min(distrib["n0"])),
                                np.log10(np.max(distrib["n0"])),
                            ),
                        )
                        hist_dist = rv_histogram(hist)
                    ...
                    if load_distrib and "eps_b" in distrib.colnames:
                        ...
                        # plt.show()
                        hist = np.histogram(
                            np.log10(distrib["eps_b"]),
                            bins=20,
                            density=True,
                            range=(
                                np.log10(np.min(distrib["eps_b"])),
                                np.log10(np.max(distrib["eps_b"])),
                            ),
                        )
                        hist_dist = rv_histogram(hist)
                    ...
                    if load_distrib and "eps_e" in distrib.colnames:
                        hist = np.histogram(
                            np.log10(distrib["eps_e"]),
                            bins=20,
                            density=True,
                            range=(
                                np.log10(np.min(distrib["eps_e"])),
                                np.log10(np.max(distrib["eps_e"])),
                            ),
                        )
                        hist_dist = rv_histogram(hist)
                    ...
                    if load_distrib and "p" in distrib.colnames:
                        hist = np.histogram(
                            ...
                            bins=20,
                            density=True,
                            range=(np.min(distrib["p"]), np.max(distrib["p"])),
                        )
                        hist_dist = rv_histogram(hist)
                    ...
                    if load_distrib and "eta" in distrib.colnames:
                        hist = np.histogram(
                            ...
                            bins=20,
                            density=True,
                            range=(np.min(distrib["eta"]), np.max(distrib["eta"])),
                        )
                        hist_dist = rv_histogram(hist)
                    ...
                    if load_distrib and "E_iso" in distrib.colnames:
                        hist = np.histogram(
                            np.log10(distrib["E_iso"]),
                            bins=20,
                            density=True,
                            range=(
                                np.log10(np.min(distrib["E_iso"])),
                                np.log10(np.max(distrib["E_iso"])),
                            ),
                        )
                        hist_dist = rv_histogram(hist)
                    ...
                    if load_distrib and "E_K" in distrib.colnames:
                        hist = np.histogram(
                            np.log10(distrib["E_K"]),
                            bins=20,
                            density=True,
                            range=(
                                np.log10(np.min(distrib["E_K"])),
                                np.log10(np.max(distrib["E_K"])),
                            ),
                        )
                        hist_dist = rv_histogram(hist)
                    ...
                    if load_distrib and "Y" in distrib.colnames:
                        hist = np.histogram(
                            ...
                            bins=20,
                            density=True,
                            range=(np.min(distrib["Y"]), np.max(distrib["Y"])),
                        )
                        hist_dist = rv_histogram(hist)
                    ...
                    if load_distrib and "Mdot_loss" in distrib.colnames:
                        hist = np.histogram(
                            ...
                            density=True,
                            range=(
                                np.min(distrib["Mdot_loss"]),
                                np.max(distrib["Mdot_loss"]),
                            ),
                        )
                        hist_dist = rv_histogram(hist)
                    ...
                    if load_distrib and "Vw" in distrib.colnames:
                        hist = np.histogram(
                            ...
                            bins=20,
                            density=True,
                            range=(np.min(distrib["Vw"]), np.max(distrib["Vw"])),
                        )
                        hist_dist = rv_histogram(hist)
                    ...
                    if load_distrib and "ism_type" in distrib.colnames:
                        hist = np.histogram(
                            ...
                            density=True,
                            range=(
                                np.min(distrib["ism_type"]),
                                np.max(distrib["ism_type"]),
                            ),
                        )
                        hist_dist = rv_histogram(hist)
                ...
                eps_b = np.array(
                    [float("{0:.2e}".format(x)) for x in eps_b], dtype=float
                )
                ...
                Mdot_loss = np.array(
                    ["{0:.2f}".format(x) for x in Mdot_loss], dtype=float
                )
                Vw = np.array(["{0:.2f}".format(x) for x in Vw], dtype=float)
                eta = np.array(["{0:.2f}".format(x) for x in eta], dtype=float)
                ism_type = np.array(
                    ["{0:d}".format(int(x)) for x in ism_type], dtype=int
                )

            # print (np.log10(n0))
            Av_samples = np.array(
                ["{0:.2f}".format(x) for x in Av_samples], dtype=float
            )
        ...
        if model == "kann":
            ...
            self.plot_distrib(
                np.log10(eps_b), 20, plot_distribution, "eps_b", log=False
            )
            self.plot_distrib(
                np.log10(eps_e), 20, plot_distribution, "eps_e", log=False
            )
            ...
            self.plot_distrib(
                np.log10(E_iso * (1 - eta) / eta), 20, plot_distribution, "E_K"
            )
        ...
        ascii.write(
            self.grb_params, self.respath + "Params.tex", format="latex", overwrite=True
        )
        ascii.write(self.grb_params, self.respath + "Params.txt", overwrite=True)
        if load_params == False:
            ascii.write(self.grb_params, "test.tex", format="latex", overwrite=True)
    ...
    def gen_distribution(self, params):
        ...
        if distype == "constant":
            ...
            x = halfnorm.rvs(
                loc=mean, scale=std, size=num_samples
            )  # ,random_state=None)
    ...
    def plot_distrib(self, distrib, num_bins, disp, label, log=False):
        ...
        n, bins, patches = plt.hist(
            distrib, bins=int(num_bins), density=False, facecolor="c"
        )  # ,alpha=0.5)
        ...
        if label == "Av_host":
            ...
            newlabel = "$log_{10}$ $(\epsilon_{B})$"
        elif label == "eps_e":
            newlabel = "$log_{10}$ $(\epsilon_{e})$"
        elif label == "E_iso":
            newlabel = "$log_{10}$ $(E_{iso})$"
        elif label == "E_K":
            newlabel = "$log_{10}$ $(E)$"
        elif label == "eta":
            newlabel = "$log_{10}$ $(\eta)$"
        elif label == "n0":
            newlabel = "$log_{10}$ $(n_0)$"
        else:
            newlabel = label
    ...
    def set_lightcurves(
        self,
        SNR4detection=3,
        err_mag_sys=[0.04, 0.06],
        configFile="Test_mock.hjson",
        telescope="colibri",
        etc_plot=False,
        disp=False,
        verbose=False,
        ...
        fname="",
        RA_J2000="0h54m50.6s",
        DEC_J2000="+14d05m04.5s",
        galactic_dust_corrected=1,
        t_since_burst=60,
        t_start=0,
        t_end=300,
        t_dead=5,
        ...
        for p in range(p1):
            ...
            self.set_strategy_obs(
                load_strategy=False,
                t_since_burst=t_since_burst,
                t_start=t_start,
                t_end=t_end,
                t_dead=t_dead,
                texp=texp,
            ...
            for j in range(n):
                ...
                if GRB_detection_status == "detected":
                    ...
                    if j < n - 1:
                        while (
                            obs_table["Nb_obs"][j - 1]
                            == obs_table["Nb_obs"][j + counter]
                        ):
                        ...
                        self.set_strategy_obs(
                            load_strategy=False,
                            t_since_burst=obs_table["time_since_burst"][j - 1],
                            t_start=obs_table["time_since_burst"][j + counter]
                            - obs_table["time_since_burst"][0],
                            t_end=t_end,
                            t_dead=t_dead,
                            texp=texp,
                            filter_set=filter_set,
                        ...
                        for i in range(len(obs_table[j + counter :])):
                            obs_table["Name"][j + counter + i] = self.grb_params[
                                "name"
                            ][p]
                        # obs_table['Name']=self.grb_params['name'][p]
                ...
                etc_GFT.information["t_sinceBurst"] = (
                    obs_table["time_since_burst"][j] / 86400
                )
                ...
                print(
                    self.grb_params["name"][p],
                    obs_table["time_since_burst"][j],
                    obs_table["band"][j],
                )
                if SNR >= SNR4detection:
                    err_mag = np.sqrt(
                        (float(2.5 * np.log10(1.0 + 1.0 / SNR))) ** 2 + (err_sys) ** 2
                    )
                    # Draw the magitude from a gaussian distribution
                    # mag=np.random.normal(mag, err_mag, 1)
                    mag = mag
                    detection = 1
                    if GRB_detection_status == "not":
                        GRB_detection_status = "detected"
                    # print (float(2.5*np.log10(1.+1./SNR)),gft_dict['err_min'][band],err_mag)
                elif SNR < SNR4detection:
                    err_mag = np.sqrt(
                        (float(2.5 * np.log10(1 + 1.0 / SNR4detection))) ** 2
                        + (err_sys) ** 2
                    )
            ...
            self.format_outputs(
                obs_table,
                resdir=self.path_pyGRBz + resdir,
                fname="",
                RA_J2000=RA_J2000,
                DEC_J2000=DEC_J2000,
                galactic_dust_corrected=1,
    ...
    def sources_extraction(self, image, sextractor_pars):
        ...
        sp.run(
            "sex %s -c /home/dcorre/code/etc_is/gft-sim/obs_strategy/obs_strategy/gft.sex -CATALOG_NAME %s.cat -CATALOG_TYPE ASCII_HEAD -PARAMETERS_NAME /home/dcorre/code/etc_is/gft-sim/obs_strategy/obs_strategy/gft.param -DETECT_TYPE CCD -DETECT_MINAREA %d -DETECT_THRESH %d -ANALYSIS_THRESH %d -PHOT_APERTURES %d -SATUR_LEVEL %d -MAG_ZEROPOINT %f -GAIN %f -PIXEL_SCALE %f -SEEING_FWHM %f -BACK_TYPE %s -BACK_VALUE %f -BACK_SIZE %d -BACKPHOTO_TYPE %s -BACKPHOTO_THICK %d -BACK_FILTTHRESH %f -CHECKIMAGE_TYPE %s -CHECKIMAGE_NAME %s -FILTER %s -FILTER_NAME %s"
            % (
                image,
                cat_name,
                detect_minarea,
    ...
    def set_lightcurves_images(
        ...
        if not os.path.exists(respath_images):
            os.makedirs(respath_images)
        if not os.path.exists(respath_is):
            os.makedirs(respath_is)
        ...
        filter_sex = "Y"
        filter_name = (
            "/home/corre/codes/astromatic/sextractor-2.19.5/config/gauss_1.5_3x3.conv"
        )
        ...
        for p in range(p1):
           ...
           """
            ...
            for j in range(n):
               ...
               """
                ...
                if obs_table["band"][j] in self.filter_vis1:
                    ...
                    GFT_IS.config["VignettingFile"] = (
                        "%s/Vignetting_vis.fits" % self.resdir
                    )
                    ...
                    GFT_IS.config["VignettingFile"] = (
                        "%s/Vignetting_nir.fits" % self.resdir
                    )
                ...
                GFT_IS.config["SourcesList"]["file"] = "%s/Sources.txt" % (
                    self.resdir
                )  # ,obs_table['band'][j])
                GFT_IS.config["output"] = "%s/%s/%s_%ss.fits" % (
                    self.resdir,
                    obs_table["Name"][0],
                    obs_table["band"][j],
                    obs_table["time_since_burst"][j],
                ...
                GFT_IS.config["PSF"]["total"][
                    "file"
                ] = "total_PSF/%s/PSF_total_%s.fits" % (
                    self.resdir,
                    obs_table["band"][j],
                )
                ...
                self.sources_extraction(
                    GFT_IS.path + "/images/" + GFT_IS.config["output"], sextractor_pars
                )
                ...
                mask = (
                    (sex_cat["XPEAK_IMAGE"] >= 150 - offset)
                    & (sex_cat["XPEAK_IMAGE"] <= 150 + offset)
                    & (sex_cat["YPEAK_IMAGE"] >= 150 - offset)
                    & (sex_cat["YPEAK_IMAGE"] <= 150 + offset)
                )
                ...
                if mask.any():
                    ...
                    print(
                        "ISO: %.2f +/- %.2f"
                        % (sex_cat[mask]["MAG_ISO"], sex_cat[mask]["MAGERR_ISO"])
                    )
                    print(
                        "ISOCORR: %.2f +/- %.2f"
                        % (sex_cat[mask]["MAG_ISOCOR"], sex_cat[mask]["MAGERR_ISOCOR"])
                    )
                    print(
                        "APER: %.2f +/- %.2f"
                        % (sex_cat[mask]["MAG_APER"], sex_cat[mask]["MAGERR_APER"])
                    )

                    print(
                        "AUTO: %.2f +/- %.2f"
                        % (sex_cat[mask]["MAG_AUTO"], sex_cat[mask]["MAGERR_AUTO"])
                    )
                    print(
                        "SNR: %.2f"
                        % (sex_cat[mask]["FLUX_AUTO"] / sex_cat[mask]["FLUXERR_AUTO"])
                    )

                    mag, mag_err = (
                        sex_cat[mask]["MAG_ISOCOR"],
                        sex_cat[mask]["MAGERR_ISOCOR"],
                    )
            ...
            if self.num_samples == 1 or p == 0:
                self.observations = obs_table.copy()
            else:
                self.observations = vstack([self.observations, obs_table.copy()])
            obs_table = None
            # del obs
            # del
            plt.close("all")
...
#z:%s
#Av_host:%s
time_since_burst band flux flux_err Texp zp flux_unit detection telescope
""" % (
                obs["Name"].data[0],
                str(RA_J2000),
                str(DEC_J2000),
                galactic_dust_corrected,
                self.grb_params["z"][
                    self.grb_params["name"] == obs["Name"].data[0]
                ].data[0],
                self.grb_params["Av_host"][
                    self.grb_params["name"] == obs["Name"].data[0]
                ].data[0],
            )
            ...
            for line in obs:
                f.write(
                    "%.2f %s %.4f %.4f %.2f %s %s %d %s\n"
                    % (
                        line["time_since_burst"],
                        line["band"],
                        line["mag"],
                        line["mag_err"],
                        line["Exp_time"],
...
if __name__ == "__main__":
    ...
    a.set_grb_params(
        ...
        load_params=True,
        params_file="test_wp2.tex",
        load_min=201,
        load_max=300,
        load_distrib=False,
        distrib_file="Granot_Sari/Turpin_GS02_v2_Ek.txt",
